...
class EnhancedMusicGenerator:
    ...
    def generate_dynamic_prompts(self, analysis: Dict) -> List[types.WeightedPrompt]:
        """Generate smart adaptive prompts that sync with video content"""
        dynamic_prompts = []
        
        # Extract all relevant analysis data
        ...
        # 1. Rhythm Section Based on Motion
        if motion > 0.6:
            # High motion - intense rhythm
            dynamic_prompts.extend([
                types.WeightedPrompt(text="kick", weight=3.5),
                types.WeightedPrompt(text="high energy", weight=3.0)
            ])
        elif motion > 0.3:
            # Medium motion - moderate rhythm
            dynamic_prompts.extend([
                types.WeightedPrompt(text="Funk Drums", weight=2.5),
                types.WeightedPrompt(text="Bongos", weight=2.0)
            ])
        else:
            # Low motion - subtle rhythm
            dynamic_prompts.extend([
                types.WeightedPrompt(text="minimal beats", weight=1.5),
                types.WeightedPrompt(text="soft percussion", weight=1.0)
            ])

        # 2. Depth-Based Reverb and Atmosphere
        if depth_profile in ['deep', 'deep_with_detail']:
            dynamic_prompts.extend([
                types.WeightedPrompt(text="reverb", weight=4.0),
                types.WeightedPrompt(text="atmospheric", weight=3.0),
            ])
        elif depth_profile in ['medium', 'medium_with_detail']:
            dynamic_prompts.append(
                types.WeightedPrompt(text="reverb", weight=2.0)
            )
        
        # 3. Brightness-Based Tone Color
        if brightness == "bright":
            dynamic_prompts.extend([
                types.WeightedPrompt(text="bright tones", weight=2.5),
            ])
        elif brightness == "dark":
            dynamic_prompts.extend([
                types.WeightedPrompt(text="dark atmosphere", weight=2.5),
            ])
        
        # 5. Color-Based Tonal Qualities
        for color in colors[:2]:
            if color == "blue":
                dynamic_prompts.append(
                    types.WeightedPrompt(text="calm melody", weight=1.5)
                )
            elif color == "red":
                dynamic_prompts.append(
                    types.WeightedPrompt(text="energetic", weight=2.0)
                )
            elif color == "green":
                dynamic_prompts.append(
                    types.WeightedPrompt(text="natural harmony", weight=1.5)
                )

        # 6. Motion Pattern Analysis
        if hasattr(self, 'motion_window'):
            motion_pattern = np.array(self.motion_window[-5:])
            if len(motion_pattern) >= 5:
                # Detect rhythmic motion
                motion_peaks = np.diff(motion_pattern)
                if np.all(np.abs(motion_peaks) > 0.2):
                    dynamic_prompts.append(
                        types.WeightedPrompt(text="rhythmic pattern", weight=3.0)
                    )
                # Detect sudden changes
                if np.max(np.abs(motion_peaks)) > 0.5:
                    dynamic_prompts.append(
                        types.WeightedPrompt(text="dynamic transition", weight=2.5)
                    )

        # 7. Scene Context Matching (keep existing logic but refined)
        scene_desc = " ".join([scene for scene, _ in analysis['top_scenes'][:2]])
        object_desc = " ".join([obj['name'] for obj in analysis['objects'][:3]])
        env_desc = f"{brightness} {analysis.get('weather', '')} {analysis.get('time_of_day', '')}"
        ...
        # Find matching instruments and moods with higher precision
        instrument_scores = util.pytorch_cos_sim(context_emb, instrument_embs)[0]
        top_instrument_idx = torch.topk(instrument_scores, k=2).indices
        
        mood_scores = util.pytorch_cos_sim(context_emb, mood_embs)[0]
        top_mood_idx = torch.topk(mood_scores, k=2).indices
        
        # Add matched instruments and moods with adaptive weights
        for idx in top_instrument_idx:
            similarity = float(instrument_scores[idx])
            # Higher weight for very strong matches
            weight = 2.0 + (similarity * 2.0) if similarity > 0.8 else 1.0 + similarity
            dynamic_prompts.append(
        ...
        for idx in top_mood_idx:
            ...
            weight = 1.5 + (similarity * 1.5)
            dynamic_prompts.append(
                types.WeightedPrompt(
                    text=MOODS[idx],
                    weight=min(4.0, weight)
                )
            )
...
async def live_camera_processing(broadcast_func: Callable[[bytes], Awaitable[None]]):
    ...
    try:
        async with client.aio.live.music.connect(model=MODEL) as session:
            ...
            # Collect initial analysis data and generate base prompts
            base_prompts = None
            ...
            try:
                while True:
                    ...
                    # Update only dynamic prompts during live processing
                    if current_time - last_update_time > 4.0:
                        # Generate only dynamic prompts
                        dynamic_prompts = prompt_generator.generate_dynamic_prompts(analysis)
                        
                        # Combine base prompts with new dynamic prompts
                        combined_prompts = base_prompts + dynamic_prompts
                        ...
                        # Simple config updates based on motion and brightness
                        if motion > 0.6:
                            config_updates['density'] = min(0.8, current_config.density + 0.1)
                        elif motion < 0.2:
                            config_updates['density'] = max(0.4, current_config.density - 0.1)
                        ...
                        await session.set_weighted_prompts(prompts=combined_prompts)
                        last_update_time = current_time
                    
                    # Display analysis
                    status = {
                        'base_prompts_generated': True,
                        'num_base_prompts': len(base_prompts),
                        'num_dynamic_prompts': len(prompt_generator.current_dynamic_prompts),
                        'analysis_frames_collected': len(prompt_generator.initial_analysis_data)
                    }
